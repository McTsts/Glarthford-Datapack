...
execute as @s[tag=tbcEnemy,scores={tbcBlindness=1..}] if score n Random <= @s tbcEscape run tellraw @a ["",[{"nbt":"EnderItems[{Slot:0b}].components.minecraft:custom_data.name","entity":"@s","interpret":true}],{"nbt":"ArmorItems[0].components.minecraft:custom_data.CustomName","entity":"@s[type=!player]","interpret":true},{"text":" >> ","color":"gray"},{"translate":"tbc.msg.broke_free","with":[{"translate":"tbc.attack.msg.name.blindness2"}]}]
