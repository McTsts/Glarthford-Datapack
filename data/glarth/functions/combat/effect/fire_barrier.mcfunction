...
execute unless score @s tbcFireResis matches 1.. unless score @s tbcAmFire matches 1.. run tellraw @a ["",[{"nbt":"EnderItems[{Slot:0b}].components.minecraft:custom_data.name","entity":"@s","interpret":true}],{"nbt":"ArmorItems[0].components.minecraft:custom_data.CustomName","entity":"@s[type=!player]","interpret":true},{"text":" >> ","color":"gray"},{"translate":"tbc.msg.fire_barrier","with":[{"text":"2.±","color":"red"}]}]
