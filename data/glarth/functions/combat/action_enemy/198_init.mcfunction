execute if score enemiesAll tbcStats matches ..1 run scoreboard players set @s tbcAttack 198
execute if score enemiesAll tbcStats matches ..1 run tellraw @a ["",{"nbt":"ArmorItems[0].components.minecraft:custom_data.CustomName","entity":"@s","interpret":true},{"text":" >> ","color":"gray"},{"translate":"tbc.msg.use","with":[{"translate":"tbc.attack.enemy.198","bold":true}]}]
execute if score enemiesAll tbcStats matches ..1 run scoreboard players set doLaser tbcStats 1
execute if score enemiesAll tbcStats matches ..1 run scoreboard players set @s attack_198 0
