execute as @e[tag=tbcTNTSel,type=armor_stand] at @s run particle squid_ink ~ ~1 ~ 0 0 0 0.01 1
...
execute as @e[tag=tbcTNTSel,tag=func,type=armor_stand] at @s if score @e[tag=tbcTarget,limit=1] blocked matches 0 run scoreboard players set @e[tag=tbcTarget,limit=1] tbcBlindness 2
