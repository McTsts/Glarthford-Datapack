...
execute as @e[tag=tbcArrowP,tag=!tbcFGRev,type=armor_stand] at @s if entity @e[tag=tbcTargetAS,distance=..4.0,type=armor_stand] as @e[tag=tbcTarget,limit=1] unless score @s blocking matches 1 unless score @s blocked matches 1 run replaceitem entity @e[tag=tbcGhastDisModel,limit=1] armor.head diamond_hoe{CustomModelData:264}
...
execute as @e[tag=tbcArrowP,tag=func2,type=armor_stand] at @s if score n Random matches 2.. run replaceitem entity @e[tag=tbcGhastDisModel,limit=1] armor.head diamond_hoe{CustomModelData:362}
