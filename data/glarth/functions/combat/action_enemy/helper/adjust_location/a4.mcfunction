...
execute store result entity @s Pose.Head[0] float -1 run scoreboard players get nextAngle tbcAdjLoc
...
execute as @e[tag=tbcAdjLoc] at @s run tp @s ^ ^ ^-0.1
execute as @e[tag=tbcAdjLoc] at @s run tp @s ~ ~0.1 ~

# Teleport
execute at @e[type=area_effect_cloud,tag=tbcAdjLoc] run tp @s ~ ~-1.6 ~
