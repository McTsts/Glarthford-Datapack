...
execute as @e[tag=tbcFang,type=armor_stand] at @s at @e[tag=tbcTargetAS,type=armor_stand] positioned ^ ^ ^-0.5 if entity @s[distance=..0.2] run tag @s add func
...
execute as @e[tag=tbcFang,tag=func,type=armor_stand] at @s run scoreboard players add damage tbcStats 10
