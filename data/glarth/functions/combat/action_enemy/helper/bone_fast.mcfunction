execute as @e[tag=tbcBoneSel,type=armor_stand] at @s run tp @s ^ ^ ^0.2
execute as @e[tag=tbcBoneSel,type=armor_stand] at @s if entity @e[tag=tbcTargetAS,distance=..0.85,type=armor_stand] as @e[tag=tbcTarget,limit=1] unless score @s blocking matches 1 unless score @s blocked matches 1 run function glarth:combat/action/blocking_start
