...
execute as @e[tag=tbcArrowP,type=armor_stand,tag=tbcArrowSlowness] at @s run particle block snow_block ~ ~0.5 ~ 0.1 0.1 0.1 0 1
execute as @e[tag=tbcArrowP,type=armor_stand] at @s if entity @e[tag=tbcTargetAS,distance=..2.5,type=armor_stand] as @e[tag=tbcTarget,limit=1] if score @s inBlock matches 0 run function glarth:combat/action/blocking_init
execute as @e[tag=tbcArrowP,type=armor_stand] at @s if entity @e[tag=tbcTargetAS,distance=..1.6,type=armor_stand] as @e[tag=tbcTarget,limit=1] unless score @s blocking matches 1 unless score @s blocked matches 1 run function glarth:combat/action/blocking_start
...
#execute as @e[tag=tbcArrowP,tag=func,type=armor_stand] at @s run function glarth:combat/action_enemy/util/get_target (NO TARGET CHANGE!)
execute as @e[tag=tbcArrowP,tag=func,type=armor_stand] at @s if entity @e[tag=tbcArrowP,distance=0.1..] as @e[tag=tbcTarget,limit=1] run function glarth:combat/action/blocking_init
