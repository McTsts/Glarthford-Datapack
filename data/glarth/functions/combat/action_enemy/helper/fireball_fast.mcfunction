execute as @e[tag=tbcArrowP,type=armor_stand] at @s rotated ~90 ~ run tp @s ^ ^ ^0.5 ~-90 ~
...
execute as @e[tag=tbcArrowP,type=armor_stand] at @s if entity @e[tag=tbcTargetAS,distance=..3.0,type=armor_stand] as @e[tag=tbcTarget,limit=1] unless score @s blocking matches 1 unless score @s blocked matches 1 run function glarth:combat/action/blocking_start
execute as @e[tag=tbcArrowP,type=armor_stand] at @s if entity @e[tag=tbcTargetAS,distance=..0.4,type=armor_stand] run tag @s add func
