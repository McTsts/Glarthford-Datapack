...
execute unless entity @e[tag=tbcEnemy6] unless entity @e[tag=tbcEnemy7] run tellraw @a ["",{"nbt":"ArmorItems[0].tag.CustomName","entity":"@s","interpret":true},{"text":" >> ","color":"gray"},{"translate":"tbc.msg.use","with":[{"translate":"tbc.attack.enemy.63","bold":true}]}]
