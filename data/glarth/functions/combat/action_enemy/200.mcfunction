...
execute if score @s tbcAttackStage matches 1 if score @s tbcAttackTimer matches 70 if score @e[tag=tbcTarget,limit=1] blocked matches 0 as @e[tag=tbcTargetAS,type=armor_stand] at @s run item replace entity @e[tag=charQ5,sort=nearest,limit=1] armor.head with diamond_hoe{CustomModelData:473}
