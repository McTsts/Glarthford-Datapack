execute if score @s tbcAttackStage matches 0 as @e[scores={tbcID=2}] at @s as @e[tag=charQE,limit=1,sort=nearest] run replaceitem entity @s armor.head diamond_hoe{CustomModelData:42}
...
execute if score @s tbcAttackStage matches 5 if score @s tbcAttackTimer matches 11 if score @e[tag=tbcTarget,limit=1] blocked matches 0 run particle minecraft:block stone ^ ^1 ^1 0 0 0 0 100
