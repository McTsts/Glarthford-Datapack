...
execute if score @s tbcAttackStage matches 1 run tag @e[tag=tbcStrWither,limit=1] remove stage_058
execute if score @s tbcAttackStage matches 1 run tag @e[tag=tbcStrWither,limit=1] remove death_028
execute if score @s tbcAttackStage matches 1 run tag @e[tag=tbcStrWither,limit=1] add stage_059
execute if score @s tbcAttackStage matches 1 run tag @e[tag=tbcStrWither,limit=1] add death_029
execute if score @s tbcAttackStage matches 1 run scoreboard players set @e[tag=tbcStrWither,limit=1] tbcMaxHealth 180
execute if score @s tbcAttackStage matches 1 run scoreboard players set @e[tag=tbcStrWither,limit=1] tbcHealth 180
execute if score @s tbcAttackStage matches 1 run data merge entity @e[tag=tbcStrWither,limit=1] {ArmorItems:[{id:"stone",count:1,components:{"minecraft:custom_data":{CustomName:'{"translate":"tbc.short_name.enemy.golem.wither","color":"dark_gray"}'}}},{},{},{id:"diamond_hoe",count:1,components:{"minecraft:custom_model_data":496}}],CustomNameVisible:0,CustomName:'{"translate":"tbc.enemy.golem.wither","color":"dark_gray"}'}
