...
execute at @e[tag=adri,type=armor_stand] if entity @e[type=wolf,distance=..7] run tellraw @a[distance=..5] ["",{"translate":"trade2","with":[{"translate":"adri.a2"}],"color":"aqua","clickEvent":{"action":"run_command","value":"/trigger trigger set 159"}}]
